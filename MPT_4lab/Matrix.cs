...
namespace MPT_4lab
{
    ...
    public class Matrix
    {
        ...
        public static Matrix operator +(Matrix a, Matrix b)
        {
            if (a.I != b.I | a.J != b.J)
            {
                throw new MyException(string.Format("Размерности матриц a и b разные"));
            }
        ...
        public Matrix Transp()
        {
            if (I != J)
            {
                throw new MyException(string.Format("Число строк и столбцов должно совпадать"));
            }
            int tmp = 0;
            for (int i = 0; i < I; i++)
            {
        ...
        public int Take_elem(int n, int m)
        {
            if (n < 0 | n > I - 1)
            {
                throw new MyException(string.Format("Неверное значение n = {0}", n));
            }
            if (m < 0 | m > J - 1)
            {
                throw new MyException(string.Format("Неверное значение m = {0}", m));
            }
            int num = 0;
            for (int i = 0; i < I; i++)
            {
